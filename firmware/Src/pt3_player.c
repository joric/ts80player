...
{
        if (i > 84*3 || (i % 3) || player->chip[0].header->PatternsPointer + 2*i+8 >= player->mod_size) return 0;
...
uint8_t PT3Player_GetTime(PT3Player * player) {
        ...
        for (int i = 0; i < player->chip[0].header->NumberOfPositions; i++) {
                if (i == player->chip[0].header->LoopPosition) player->loop = player->time;
                ...
                for (;;) {
                        if (!PT3Player_fastSimulatePattern(player, &pat[0])) break;
                        if (TS != 0x20 && !PT3Player_fastSimulatePattern(player, &pat[1])) break;
                        player->time += player->cur_delay;
                        if (--DLCatcher == 0) return 0;
                }
...
uint8_t PT3Player_Init(PT3Player * player, uint8_t * data, uint32_t data_size) {
        ...
        if (!PT3Player_GetTime(player)) return 0;
        player->tick = 0;
        ...
        for (int cnum = 0; cnum < 2; cnum++) {
                ...
                for (int chan = 0; chan < 3; chan++) {
                        player->chip[cnum].ch[chan].Address_In_Pattern =
                                player->chip[cnum].module[player->chip[cnum].header->PatternsPointer + 2*(i+chan)] +
                                player->chip[cnum].module[player->chip[cnum].header->PatternsPointer + 2*(i+chan)+1] * 0x100;
...
void PT3Player_PatternInterpreter(PT3Player * player, int cnum, PT3_Channel * chan){
        ...
        while (counter > 0) {
                if (counter == f1) {
                        ...
                        chan->Note = PrNote;
                        if (player->version >= 6) chan->Current_Ton_Sliding = PrSliding;
...
void PT3Player_ChangeRegisters(PT3Player * player, int cnum, PT3_Channel * chan)
{
        if (chan->Enabled) {
                ...
                if (player->version <= 4) chan->Amplitude = PT3VolumeTable_33_34[chan->Volume][chan->Amplitude];
                ...
                if (b1 & 0x80) {
                        ...
                        if (b1 & 0x20) chan->Current_Noise_Sliding = player->chip[cnum].mod.AddToNoise;
                }
...
void PT3Player_GetRegisters(PT3Player * player, int cnum)
{
        ...
        for (int ch = 0; ch < 3; ch++) PT3Player_ChangeRegisters(player, cnum, &player->chip[cnum].ch[ch]);
...
void PT3Player_Step(PT3Player * player) {
        ...
        if (player->tsMode) PT3Player_GetRegisters(player, 1);
        player->tick++;
}
...
int main() {
	...
	for (uint32_t t=0; t<samples; t++) {
		...
		uint32_t amp = pt3_render(t, 44100, data, size, &samples);
